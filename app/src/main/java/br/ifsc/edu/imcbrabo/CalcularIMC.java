    ...
    public void calcularImc(View v){
        ...
        if((txtPeso.getText().toString().length()>0)&&(txtAltura.getText().toString().length()>0)){
            ...
            if ((peso>0)&&(altura>0)){
                ...
                Toast.makeText(this, "Altura digitada ou peso inválido(s)", Toast.LENGTH_SHORT).show();
            }
        }
        else{
            Toast.makeText(this, "Todos os campos devem estar preenchidos para continuar a execução", Toast.LENGTH_SHORT).show();
        }
    }
}